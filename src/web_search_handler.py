...
try:
    ...
    has_ddgs = False
    logging.warning("duckduckgo-search package not available or not properly installed")
...
class WebSearchHandler:
    ...
    def _is_valid_tavily_api_key(self, api_key: str) -> bool:
        ...
        try:
            ...
            logging.info(f"Testing Tavily API key: {api_key[:5]}...{api_key[-5:]}")
            
            # Create a temporary instance to test the API key
            if TavilyWrapper == TavilySearchResults:
                # New implementation
                logging.info("Using TavilySearchResults for API key validation")
                test_wrapper = TavilyWrapper(max_results=1)
                # Make a minimal test query
                test_result = test_wrapper.invoke("test")
                logging.info(f"Tavily API key validation successful with TavilySearchResults: {len(test_result)} results")
            else:
                # Old implementation
                logging.info("Using TavilySearchAPIWrapper for API key validation")
                test_wrapper = TavilyWrapper(api_key=api_key)
                # Make a minimal test query
                test_result = test_wrapper.results("test", max_results=1)
                logging.info(f"Tavily API key validation successful with TavilySearchAPIWrapper: {len(test_result.get('results', []))} results")
    ...
    def initialize_search_providers(self):
        ...
        logging.info(f"Reinitializing search providers with default provider: {default_provider}")
        self._initialize_provider(default_provider)
                
        # Log available providers
        if self.search_providers:
            logging.info(f"Successfully initialized search provider: {', '.join(self.search_providers.keys())}")
        else:
            logging.warning(f"Failed to initialize default provider '{default_provider}'. Please check API keys.")
            ...
            if provider == 'google_serper':
                ...
                if has_ddgs:
                    try:
                        ...
                        logging.info("Initializing DuckDuckGo search with DDGS implementation")
                        ddg_client = DDGS()
                        self.search_providers['ddg_search'] = ddg_client
                        logging.info("Successfully initialized DuckDuckGo search with DDGS")
                    except Exception as e:
                        logging.error(f"Failed to initialize DuckDuckGo search with DDGS: {str(e)}")
                else:
                    ...
                    try:
                        ...
                        logging.info("Adding DuckDuckGo search wrapper to available providers")
                        self.search_providers['ddg_search'] = ddg_wrapper
                    except Exception as e:
                        logging.error(f"Failed to initialize DuckDuckGo search wrapper: {str(e)}")
                ...
                if api_key:
                    try:
                        ...
                        if self._is_valid_tavily_api_key(api_key):
                            if TavilyWrapper == TavilySearchResults:
                                # New implementation
                                self.search_providers['tavily_search'] = TavilyWrapper(max_results=5)
                            else:
                                # Old implementation
                                self.search_providers['tavily_search'] = TavilyWrapper(api_key=api_key)
                            logging.info("Tavily search provider initialized successfully")
                        else:
                            logging.error("Failed to initialize Tavily search provider: Invalid API key")
                    except Exception as e:
                        logging.error(f"Error initializing Tavily search provider: {str(e)}")
                else:
                    logging.warning("Tavily search provider requires an API key. Please add it in settings.")
                    # If Tavily is set as default but has no API key, log a specific warning
                    default_provider = self.settings.get('web_search', 'default_provider', default='google_serper')
                    if default_provider == 'tavily_search':
                        logging.warning("Tavily is set as default search provider but no API key is provided. Will fall back to another provider.")
        except Exception as e:
...
OPTIMIZED SEARCH QUERY:"""
    ...
    def search(self, query: str, provider: Optional[str] = None, max_results: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        if provider not in self.search_providers:
            ...
            logging.warning(f"Provider '{original_provider}' not available. Using '{provider}' instead.")
            
            # If the requested provider was Tavily, provide more specific guidance
            if original_provider == 'tavily_search':
                logging.warning("To use Tavily search, please ensure you have added a valid Tavily API key in settings.")
        ...
        try:
            ...
            if provider == 'google_serper':
                ...
                logging.info(f"Google Serper search returned {len(search_results)} results")
                return [
                    {
                ...
                logging.info(f"Brave search returned {len(web_results)} results")
                return [
                    {
                        'title': result.get('title', ''),
                ...
                try:
                    ...
                    logging.info(f"Calling DuckDuckGo search with query: {query}, max_results: {max_results}")
                    
                    # Check if we're using the DDGS implementation
                    if has_ddgs and isinstance(search_provider, DDGS):
                        logging.info("Using DDGS implementation for DuckDuckGo search")
                        try:
                            # Check if cancellation was requested
                            if self._cancellation_requested:
                            ...
                            if not search_results:
                                logging.warning("DuckDuckGo search returned no results")
                                return []
                    ...
                    if isinstance(search_provider, DuckDuckGoSearchAPIWrapper):
                        logging.info("Using DuckDuckGoSearchAPIWrapper for search")
                        try:
                            # Check if cancellation was requested
                            if self._cancellation_requested:
                                logging.info("DuckDuckGoSearchAPIWrapper search cancelled")
                                return []
                                
                            search_results = search_provider.results(query, max_results=max_results)
                            ...
                            logging.error(f"Error executing DuckDuckGo search: {str(api_error)}")
                    ...
                    logging.error(f"DuckDuckGo search provider is not a recognized type: {type(search_provider)}")
                    return []
                ...
                try:
                    ...
                    if hasattr(results, 'results'):
                        ...
                        logging.info(f"Exa search returned {len(search_results)} results")
                        ...
                        for result in search_results:
                            try:
                                ...
                                logging.error(f"Error processing Exa search result: {str(e)}")
                                ...
                                try:
                                    ...
                                    logging.error(f"Failed to process Exa result even with fallback method")
                        
                        return formatted_results
                    else:
                        # Handle older API or different response format
                        logging.info("Exa search returned results in a different format")
                        ...
                        try:
                            ...
                            for result in results:
                                ...
                                if self._cancellation_requested:
                                    logging.info("Exa search result processing cancelled")
                                    return formatted_results
                                ...
                                try:
                                    ...
                                    logging.error(f"Error processing individual Exa result: {str(e)}")
                            
                            return formatted_results
                        except Exception as e:
                            logging.error(f"Failed to process Exa search results: {str(e)}")
                            return []
                except Exception as e:
                ...
                try:
                    ...
                    if not hasattr(search_provider, 'results') and not hasattr(search_provider, 'invoke'):
                        logging.error("Tavily search provider does not have a valid method. This may indicate an initialization issue.")
                        return []
                    
                    # Set the environment variable for Tavily
                    os.environ["TAVILY_API_KEY"] = self.settings.get_api_key('tavily_search')
                    ...
                    if hasattr(search_provider, 'invoke'):
                        ...
                        logging.info(f"Tavily search returned {len(results)} results")
                        ...
                        if 'results' not in results:
                            logging.error(f"Unexpected Tavily search response format: {results}")
                            return []
                        
                        logging.info(f"Tavily search returned {len(results.get('results', []))} results")
                        return [
                            {
                                'title': result.get('title', ''),
                                'snippet': result.get('content', ''),
    ...
    def scrape_content(self, urls: List[str]) -> List[Document]:
        ...
        max_urls = self.settings.get('web_search', 'max_scrape_urls', default=5)
        urls = urls[:max_urls]
