...
class MarkdownRenderer(QWidget):
    ...
    def _setup_html_template(self):
        ...
        possible_paths = [
            ...
            os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', 'assets', 'js', 'marked.min.js')
        ]
        ...
        for path in possible_paths:
            try:
                ...
                logging.debug(f"Could not load marked.js from {path}: {str(e)}")
        
        if marked_js_content is None:
            logging.error("Could not find marked.js in any of the expected locations")
    ...
    def set_markdown(self, markdown_text):
        ...
        if not isinstance(markdown_text, str):
            logging.warning(f"Expected string for markdown_text, got {type(markdown_text)}")
