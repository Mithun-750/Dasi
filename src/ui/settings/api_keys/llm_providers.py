...
class LLMProvidersSection(APIKeySection):
    ...
    def init_ui(self):
        ...
        if self.custom_openai:
            ...
            custom_title_container.setStyleSheet(
                "background-color: transparent;")
            custom_title_layout = QHBoxLayout(custom_title_container)
            custom_title_layout.setContentsMargins(
                0, 20, 0, 0)  # Add top margin for spacing
            # Add spacing between title and button
            custom_title_layout.setSpacing(10)

            # Add section title
            self.custom_openai_title = QLabel(
