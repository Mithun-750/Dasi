...
def install_dependencies():
    ...
    if platform.system() == "Windows":
        try:
            ...
            print(
                "win32com not found. Installing pywin32 for Windows shortcut support...")
...
def create_launcher():
    ...
    if system == "Linux":
        desktop_entry_path = os.path.join(os.path.expanduser(
            "~"), ".local", "share", "applications", "dasi.desktop")
...
"""
        ...
        try:
            subprocess.check_call(
                ["update-desktop-database", os.path.dirname(desktop_entry_path)])
        ...
        try:
            ...
            executable_path = os.path.join(
                current_dir, "dist", "dasi", "dasi.exe")
...
def initialize_prompt_chunks():
    ...
    default_chunks_dir = os.path.join(
        PROJECT_ROOT, "defaults", "prompt_chunks")
    if not os.path.exists(default_chunks_dir):
        logging.warning(
            f"Default prompt chunks directory does not exist: {default_chunks_dir}")
        return
    ...
    if not config_dir:
        logging.error(
            "Could not determine configuration directory for prompt chunks.")
        return
    ...
    if os.path.exists(target_chunks_dir) and os.listdir(target_chunks_dir):
        response = input(
            f"Existing configuration found at {target_chunks_dir}. Reset to defaults? [y/N]: ").strip().lower()
    ...
    for config_path in qt_config_paths:
        if os.path.isfile(config_path):
            ...
            try:
                ...
                app_configs = glob.glob(
                    os.path.join(config_path, f"*{app_name}*"))
                ...
                print(
                    f"Could not process Qt config directory {config_path}: {e}")
    ...
    if system == "Darwin":  # macOS
        cache_paths = [
            ...
            os.path.join(os.path.expanduser("~"), "Library",
                         "Caches", "QtWebEngine"),
            os.path.join(os.path.expanduser("~"), "Library",
                         "Application Support", "QtProject"),
            os.path.join(os.path.expanduser("~"), "Library",
                         "Preferences", f"com.{app_name}.plist")
        ]
        ...
        if os.environ.get("APPDATA"):
            appdata_paths.append(os.path.join(
                os.environ.get("APPDATA"), app_name))
            appdata_paths.append(os.path.join(
                os.environ.get("APPDATA"), "QtProject"))

        if os.environ.get("LOCALAPPDATA"):
            appdata_paths.append(os.path.join(
                os.environ.get("LOCALAPPDATA"), app_name))
            appdata_paths.append(os.path.join(
                os.environ.get("LOCALAPPDATA"), "QtProject"))
...
def uninstall():
    ...
    if config_dir and os.path.exists(config_dir):
        ...
        response = input(
            f"Do you want to remove configuration directory at {config_dir}? [y/N]: ").strip().lower()
        if response == 'y' or response == 'yes':
    ...
    response = input(
        "Do you want to clean OS-level cache files? [y/N]: ").strip().lower()
    if response == 'y' or response == 'yes':
        clean_os_cache()
